...
- **spell_book.py**: Contains the function that can be used to add new spells to the CLI. It also retroactively adds all the spells in the 'spells.py' file to the .tome folder.

## Usage

Magi CLI can be run without prefacing each command with 'python magi_cli.py' by installing with pip. First navigate to the root directory and run

```bash
pip install .
```

After that it should be installed. Just enter cast, then the spell name and the required arguments to use it.
